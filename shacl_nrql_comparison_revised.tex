...
The comparison so far asked what \shacl{} \emph{added}. The converse is just as telling, and sharper than the prose of \S\ref{sec:stronger} suggests: a substantial part of the \nrql{}/\racer{} repertoire has no \shacl{} counterpart even today, because \shacl{} is by design a validator over a fixed graph, not a reasoner. To make the point concrete rather than rhetorical, every row of Table~\ref{tab:cantdo} was \emph{executed live} on a \racer{}~2.0 server; the transcripts are in the accompanying repository (\texttt{verification/reasoning\_demos.py}). For good measure the first two rows were also reproduced from an actual \emph{loaded \owl{} file}: \racer{} parsed an ontology defining $\texttt{DogOwner}\equiv\texttt{Person}\sqcap\exists\texttt{owns.Dog}$, classified $\texttt{DogOwner}\sqsubseteq\texttt{PetOwner}$ (never asserted, from $\texttt{Dog}\sqsubseteq\texttt{Pet}$), and realized the lone individual---asserted only as a \texttt{Person} owning a \texttt{Dog}---as both a \texttt{DogOwner} and a \texttt{PetOwner} (\texttt{verification/owl\_classification.py}).
...
\bibitem{racerug}
Racer Systems GmbH \& Co.\ KG. \emph{RacerPro User's Guide Version 2.0}. October 24, 2012. \url{https://github.com/lambdamikel/shacl-nrql-comparison/blob/main/manuals/RacerPro-Users-Guide-2.0.pdf}. (Page numbers in this report are the printed page numbers of this edition; syntax was verified against \S4.1.9, pp.~150--153.)

\bibitem{racerrm}
Racer Systems GmbH \& Co.\ KG. \emph{RacerPro Reference Manual Version 1.9}. October 12, 2010. \url{https://github.com/lambdamikel/shacl-nrql-comparison/blob/main/manuals/RacerPro-Reference-Manual-1.9.pdf}.
