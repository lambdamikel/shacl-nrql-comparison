...
\subsection{Both sides have a Turing-complete escape hatch}\label{sec:hatch}
The expressivity comparison must be conducted at the right level, because \emph{both} languages provide a general-purpose escape hatch. \shacl{} has \shacl{}-SPARQL and \shacl{}-JS, which embed arbitrary SPARQL or JavaScript. \nrql{} has \emph{functional lambda expressions}: a \texttt{retrieve1} query whose head contains a \texttt{(:lambda \dots)} together with the \texttt{:reject} token defines, in the manual's words, ``arbitrary, user-defined filter predicates which are executed efficiently \dots\ directly on the RacerPro server'' \cite[pp.~146--148]{racerug}. The lambda body is full Common Lisp---the manual's own examples open files, fold over lists, and call back into \texttt{retrieve} and \texttt{tbox-retrieve1}---so it is Turing-complete with callback into the knowledge base.

This reshapes the comparison. First, the regex gap of \S\ref{sec:empirical} disappears: a lambda filter calling a Common Lisp regular-expression library implements \texttt{sh:pattern} including flags, exactly as a \shacl{}-SPARQL \texttt{FILTER regex(\dots)} does; by the same mechanism term-kind, language-tag, and lexical-length checks are all expressible. Second, and more sharply, even the unbounded-path residual of Proposition~\ref{prop:paths} can be \emph{computed} by a lambda that performs a hand-coded graph traversal via nested \texttt{retrieve} calls---though at that point one has written a reachability algorithm in Lisp, abandoning the declarative query semantics, just as a hand-rolled \shacl{}-SPARQL transitive-closure query would. The honest conclusion is therefore tiered:
\begin{itemize}[leftmargin=2em]
  \item \textbf{Declarative \nrql{} vs.\ \shacl{} Core.} The residual is the $\approx 5$--$8\%$ measured in \S\ref{sec:empirical}: unbounded regular paths (provably, Proposition~\ref{prop:paths}) and full-regex \texttt{sh:pattern}. \shacl{} Core states these \emph{declaratively}; declarative \nrql{} cannot, short of a transitive-role declaration or a recursive rule.
  \item \textbf{Full \nrql{} (lambda/\texttt{:reject}) vs.\ \shacl{}-SPARQL/JS.} Both are Turing-complete escape hatches with KB access; at this tier neither is more expressive and the \shacl{}-Core expressivity residual collapses to nothing. The comparison ceases to be about expressivity and becomes about declarativity, ergonomics, and standardization.
\end{itemize}
So the residual that survives \emph{both} escape hatches is not an expressivity gap at all: it is \shacl{}'s recursive shape-conformance \emph{semantics} (\S\ref{sec:recursion}) and validation-report \emph{model} (\S\ref{sec:new}.5), plus the fact that \shacl{} Core offers regular paths as \emph{declarative} primitives where \nrql{} offers them only through roles, rules, or the host language.
...
\end{quote}
The width of the band is set by how much \rdf{}-term and lexical structure one assumes mirrored (\S\ref{sec:empirical}); that dependence on assumptions is the honest replacement for a single number. Crucially, the part that is \emph{provably} beyond nonrecursive \nrql{} (Proposition~\ref{prop:paths}) does not move with those assumptions: it is the unbounded-path category. A second, smaller gap at the declarative level---\texttt{sh:pattern} regular expressions---also does not depend on mirroring, since RacerPro's concrete domain offers only \texttt{string=}/\texttt{string<>} and the substrate a substring \texttt{search}; but that is an automaton-matching matter rather than a finite-model-theoretic one, and unlike the paths it is closed by \nrql{}'s lambda escape hatch (\S\ref{sec:hatch}). Indeed \emph{both} languages carry a Turing-complete escape hatch---\nrql{} lambdas with \texttt{:reject}, \shacl{}-SPARQL/JS---so the headline band describes the \emph{declarative} comparison; at the escape-hatch tier the expressivity residual vanishes and what remains is \shacl{}'s conformance semantics and report model. We carry out the coding for the W3C \shacl{} test suite in \S\ref{sec:empirical}.
